...
Item {
  ...
  // Local change: upstream marks the three injected properties `required`,
  // which only the built-in bar can satisfy. A bar coming from a plugin is
  // loaded from a URL and configured in the loader's onLoaded, so a required
  // property leaves the Loader in Loader.Error and the bar never maps. They
  // carry empty defaults here and the host overwrites them a moment later.
  //
  ...
  component CenterGestureArea: MouseArea {
    ...
    // Local change: upstream toggles bar transparency here. A double click
    // lands on the bar by accident far more often than on purpose, so the
    // gesture is swallowed. Set bar.transparent in shell.json instead.
