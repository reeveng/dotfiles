...
// The menu a row sits in is part of what the row is called. Without it a
// search for "install package" asks for a row labelled both words and there
// is none, and a search for "install" answers with the Install menu and none
// of the things inside it. With it, each word of a query may land on the
// label, on an alias, or on any menu above it, so "install package" is the
// path spoken aloud, "install" is everything under Install, and "ins p" is
// the same query typed by somebody in a hurry.
function pathSearchText(entry, path) {
  return [nameSearchText(entry), searchableToken(path).toLowerCase()].join(" ")
}

function matchesQuery(entry, query, visible, path) {
  if (!entry || entry.id === "root") return false
  if (!visible) return false

  var searchText = pathSearchText(entry, path)
  var descriptionText = String(entry.description || "").toLowerCase()
  var terms = String(query || "").toLowerCase().trim().split(/\s+/)

  for (var i = 0; i < terms.length; i++) {
    if (!terms[i]) continue
    if (searchText.indexOf(terms[i]) >= 0) continue
...
// How well one word sits on a row, lowest first. Split out of searchScore so
// a query of several words can be scored on the word it is aiming at.
function needleScore(entry, needle, label, nameText, searchText, descriptionText) {
  if (label === needle) return entry.parent === "root" ? 2 : 0
  ...
  if (entry.kind === "app" && label.split(/\s+/).indexOf(needle) >= 0) return 0
  if (label.indexOf(needle) === 0) return 10
  if (label.indexOf(needle) >= 0) return 30
  if (nameText.indexOf(needle) >= 0) return 40
  if (searchText.indexOf(needle) >= 0) return 50
  if (descriptionTextMatches(needle, descriptionText)) return 60
  return 80
}
...
function searchScore(items, entry, query) {
  ...
  var searchText = pathSearchText(entry, parentPathFor(items, entry.id))
  var descriptionText = String(entry.description || "").toLowerCase()
  var score = needleScore(entry, needle, label, nameText, searchText, descriptionText)

  // "install package" names a path, and no row is labelled both words. The
  // last word is the one being aimed at, so it decides the rank while the
  // earlier words have already done their work narrowing the list.
  var words = needle.split(/\s+/)
  if (score === 80 && words.length > 1) {
    score = needleScore(entry, words[words.length - 1], label, nameText, searchText, descriptionText)
  }

  var kind = 0
  if (entry.kind === "menu" || entry.kind === "link") kind = -2
  ...
  if (entry.kind === "app") kind = -5

  // How well the row answers comes first, then how near the top of the tree it
  // sits, and only then what kind of thing it is. Depth outranks kind because
  // a shallow row reads as the general answer and the general answer is nearly
  // always the one being asked for; a submenu three levels down should not
  // come above the plain action a level up just for being a submenu. Each
  // place is wide enough that the one below it can never reach into it.
  return score * 100000 + depthFor(items, entry.id) * 10000 + kind * 1000 + entry.order
}
