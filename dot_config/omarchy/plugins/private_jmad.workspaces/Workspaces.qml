...
BarWidget {
  ...
  // Stock omarchy.workspaces gives every number a full bar-width square with a
  // gap on top. Down a side bar that reads as a loose ladder, so both the gap
  // and the square are settings here, tuned from shell.json rather than code.
  ...
  GridLayout {
    ...
    Repeater {
      ...
      WidgetButton {
        ...
        horizontalMargin: 6
        verticalPadding: root.vertical ? 0 : 6
        fixedWidth: root.vertical ? root.barSize : Style.space(20)
        fixedHeight: root.vertical ? root.buttonSize : root.barSize
