...
Panel {
  ...
  // Stock omarchy.power drops the percentage on a vertical bar, where "97% 󰂀"
  // will not fit a 28px column. A side bar has the other axis to spend, so
  // stack the number under the glyph the way the clock stacks HH over mm.
  // Panel does not carry the bar geometry that BarWidget does, so read it off
  // the host rather than off `vertical`, which only exists inside the button.
  readonly property bool barVertical: bar ? bar.vertical : false
  readonly property int barPixels: bar ? bar.barSize : Style.bar.sizeHorizontal
  readonly property bool stackPercentage: root.showPercentage && barVertical
  readonly property string percentageText: Math.round(root.batteryFraction * 100) + "%"
  ...
  Component {
    ...
    Column {
      ...
      Text {
        ...
        color: root.bar ? root.bar.foreground : Color.foreground
        font.family: root.bar ? root.bar.fontFamily : Style.font.family
        // Three characters have to clear the column's inner width. Deriving
        // the size from the bar keeps it right when the bar or font scales.
        font.pixelSize: Math.max(8, Math.round(root.barPixels * 0.36))
      }
    }
  }
  ...
  BarIconButton {
    ...
    anchors.fill: parent
    bar: root.bar
    iconComponent: root.stackPercentage ? stackedBattery : null
    text: root.showPercentage && !vertical
      ? root.percentageText + " " + root.batteryIcon()
      : root.batteryIcon()
    slotSize: Style.bar.iconSlot * (root.showPercentage ? 2 : 1)
    opticalSize: root.stackPercentage ? Style.bar.iconCanvas * 2 : Style.bar.iconCanvas
    tooltipText: ""
